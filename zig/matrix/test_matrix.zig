...
test "not a matrix" {
    const s = "1 2 3\n2 3\n4 5 6";
    const actual = row(testing.allocator, s, 3);
    defer if (actual) |a| testing.allocator.free(a) else |_| {};
    try testing.expectError(matrix.MatrixValidator.Error.RaggedMatrix, actual);
}

test "column out of range" {
    const s = "1 2 3\n4 5 6\n7 8 9";
    const actual = column(testing.allocator, s, 4);
    defer if (actual) |a| testing.allocator.free(a) else |_| {};
    try testing.expectError(matrix.MatrixValidator.Error.OutOfRange, actual);
}

test "row out of range" {
    const s = "1 2 3\n4 5 6\n7 8 9";
    const actual = row(testing.allocator, s, 4);
    defer if (actual) |a| testing.allocator.free(a) else |_| {};
    try testing.expectError(matrix.MatrixValidator.Error.OutOfRange, actual);
}
